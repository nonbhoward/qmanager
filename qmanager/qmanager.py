...
class Qmanager:
    ...
    def run(self, run_frequency):
        while True:
            if qbit_is_running():
                try:
                    self.main_loop(run_frequency)
                except Exception as exc:
                    # TODO dump exc stack to log
                    print(f'crash : {exc}')
                    raise exc
            query_qbit_application_wait_(run_frequency)

    @announce_duration
    def main_loop(self, run_frequency):
        # get qbit instance
    ...
    def apply_rename_map(self):
        ...
        # update name_maps w oldest_name info
        ehm_names_oldest = \
        ...
        for e_hash, oldest_name in ehm_names_oldest.items():
            if not isinstance(name_maps[e_hash], dict):
                ...
                name_maps[e_hash].update({e_hash: oldest_name})

        # reset names that are still in testing
        self.reset_testing_names_to_original(name_maps=name_maps)

        cds_now = self.get_latest_snapshot()
        guid_offset = self.read_guid_offset()
        if not name_maps:
            print(f'no name_maps file found')
            return

        # iterate through entries, apply maps
        for e_hash, e_details in cds_now.items():
            e_name = e_details['name']
            new_name = map_to_new_name_(old_name=e_name,
    ...
    def reset_testing_names_to_original(self, name_maps):
        ehm = self.cc.cache['entry_history_memory']
        names_original = get_original_names(ehm)
...
def query_qbit_application_wait_(run_frequency):
    main_loop_wait(run_frequency)
